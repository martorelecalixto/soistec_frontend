...
import '../services/incvendahotel_service.dart';
...
import 'package:sistrade/widgets/itemvendahotel_form.dart';
...
class VendaHotelForm extends StatefulWidget {
  final VendaHotel? vendahotel;
  final double? width;
  final double? height;

  const VendaHotelForm({
    super.key,
    this.vendahotel,
    this.width,
    this.height,
  ...
  @override
  _VendaHotelFormState createState() => _VendaHotelFormState();
}

class _VendaHotelFormState extends State<VendaHotelForm> {
  final _formKey = GlobalKey<FormState>();

  late VendaHotel vendaHotelAtual;
  ...
  List<ItensVendaHotel> _itensVendaHotel = [];
  ...
  void initState() {
    ...
    vendaHotelAtual = widget.vendahotel ?? VendaHotel();    
    _init();
  }
  ...
  void atualizarVendaHotel(VendaHotel novo) {
    setState(() {
      vendaHotelAtual = novo;
    });
  }

  // Se quiser que o estado reaja caso o widget pai mude o objeto
  @override
  void didUpdateWidget(covariant VendaHotelForm oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.vendahotel != oldWidget.vendahotel) {
      setState(() {
        vendaHotelAtual = widget.vendahotel ?? VendaHotel();
      });
    }
  ...
  .format(v.descontototal ?? 0.0);//v.descontototal?.toString() ?? '';
   ...
   // print('IDVENDA _carregarDadosIniciais = ' + v.idvenda.toString());

    // Carrega os itens da venda
    _itensVendaHotel = await ItemVendaHotelService.getItensVendaHotelByIdVenda(idvenda: v.idvenda ?? 0);
  ...
  void _abrirFormularioAddServico({Map<String, dynamic>? itemvendahotel,}) async {
      ...
      builder: (_) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: ItemVendaHotelForm(
            idVenda: int.tryParse(vendaHotelAtual.idvenda.toString()) ?? 0, //widget.vendahotel
    ...
    _carregarDadosIniciais();
     //_reabrirFormularioRequisicao(vendahotel: vendaHotelAtual?.toJson()); //widget.vendahotel
  ...
  void limparCampos() async {
    ...
    _itensVendaHotel = await ItemVendaHotelService.getItensVendaHotelByIdVenda(idvenda: 0);
    setState(() {});
  }
...
/*
  void _reabrirFormularioRequisicao({Map<String, dynamic>? vendahotel}) async {
    final resultado = await showDialog<bool>(
      context: context,
      barrierDismissible: false,
      builder: (_) => AlertDialog(
        ...
        content: SizedBox(
          width: 1200,
          height: 800,
          child: VendaHotelForm(
            vendahotel: vendahotel != null ? VendaHotel.fromJson(vendahotel) : null,
          ),
        ),
      ),
    );
...
*/
  ...
  void onNovo() {
    ...
    var vendaHotelAux = VendaHotel(
        ...
        atualizarVendaHotel(vendaHotelAux);
    ...
    if (!_formKey.currentState!.validate()) {
      return; // Sai da função e não executa mais nada.
        ...
        try{
          ...
          //BuscarId//
          if (idempresa != null) {
            if ((vendaHotelAtual.idvenda == null)|| (vendaHotelAtual.idvenda == 0)) {
              idReq = await IncVendaHotelService.incVendaHotel(idempresa);
              nroController.text = idReq.toString();
            }else{ 
                idReq = vendaHotelAtual.id ?? 0;
                nroController.text = idReq.toString();        
              }
          ...
          if (_formKey.currentState!.validate()) {
            final vendahotel = VendaHotel(
              idvenda: vendaHotelAtual.idvenda ?? 0,
              id: idReq,
              datavenda: dataVenda,
            ...
            bool sucesso = false;
            if ((vendaHotelAtual.idvenda == null)|| (vendaHotelAtual.idvenda == 0)) {
                //sucesso = await VendaHotelService.createVendaHotel(vendahotel);
                //print('IDREC -> ' + idReq.toString());
                //print('VENDABILHETE.ID -> ' + vendahotel.id.toString());
                final idGerado = await VendaHotelService.createVendaHotel(vendahotel);
                if (idGerado != null) {
              ...
              sucesso = await VendaHotelService.updateVendaHotel(vendahotel);
              final confirmar = await showDialog<bool>(
                context: context,
                builder: (_) => AlertDialog(
  ...
  void onExcluir(int? idvenda) async{
    if ((vendaHotelAtual.idvenda != 0)&&(vendaHotelAtual.idvenda != null)){
      ...
      if (confirmar == true) {
          try {
            if(idvenda != null){
              await VendaHotelService.deleteVendaHotel(idvenda);
    ...
    if ((id != 0)&&(id != null)){
      ...
      if (confirmar == true) {
          try {
            ...
            _carregarDadosIniciais();
           // _reabrirFormularioRequisicao(vendahotel: widget.vendahotel?.toJson());
            //Navigator.pop(context, true);
  ...
  void imprimirPDFRequisicao() async {
    if ((vendaHotelAtual.idvenda != 0)&&(vendaHotelAtual.idvenda != null)){
      ...
      final itensTabela = _itensVendaHotel.map((item) => [
        item.pax.toString(),
        item.descricao ?? '',
        item.descricao ?? '',
        item.descricao ?? '',
        item.descricao ?? '',
        'R\$ ${item.valorhotel?.toStringAsFixed(2) ?? '0,00'}',
        'R\$ ${item.valortaxa?.toStringAsFixed(2) ?? '0,00'}',
        'R\$ ${item.valortaxaservico?.toStringAsFixed(2) ?? '0,00'}',
        'R\$ ${item.valoroutros?.toStringAsFixed(2) ?? '0,00'}',
      ]).toList();      
      ...
      double totalValorTaxa = 0;
      double totalValorTaxaServico = 0;
      double totalValorOutros = 0;
      double totalGeral = 0;

      for (var item in _itensVendaHotel) {
        totalValorHotel += item.valorhotel ?? 0;
        totalValorTaxa += item.valortaxa ?? 0;
        totalValorTaxaServico += item.valortaxaservico ?? 0;
        totalValorOutros += item.valoroutros ?? 0;
      }

      totalGeral = totalValorHotel + totalValorTaxa + totalValorTaxaServico + totalValorOutros;

      final pdf = pw.Document();
      final dataAtual = DateFormat('dd/MM/yyyy').format(DateTime.now());
      final enderecoFilial = await FilialService.getFilialById(vendaHotelAtual.idfilial.toString());
      final enderecoEntidade = await EntidadeService.getEntidadeById(vendaHotelAtual.identidade.toString());
          ...
          build: (context) {
            return pw.Column(
              ...
              children: [
                ...
                pw.Text(vendaHotelAtual.observacao.toString()),
                pw.SizedBox(height: 12),
                    ...
                    ..._itensVendaHotel.map(
                      (item) => pw.TableRow(
                        ...
                        children: [
                          ...
                          pw.Padding(
                              padding: const pw.EdgeInsets.all(4),
                              child: pw.Text(item.descricao ?? '', style: pw.TextStyle(fontSize: 8))),
                          pw.Padding(
                              padding: const pw.EdgeInsets.all(4),
                              child: pw.Text(item.descricao ?? '', style: pw.TextStyle(fontSize: 8))),
                          pw.Padding(
                              padding: const pw.EdgeInsets.all(4),
                              child: pw.Text((item.descricao ?? '').length >= 3 ? item.descricao!.substring(0, 3) : (item.descricao ?? ''), style: pw.TextStyle(fontSize: 8))),
                          pw.Padding(
                              padding: const pw.EdgeInsets.all(4),
                              child: pw.Text(item.fornecedor ?? '', style: pw.TextStyle(fontSize: 8))),
                          pw.Padding(
                              padding: const pw.EdgeInsets.all(4),
                              child: pw.Text('R\$ ${item.valorhotel?.toStringAsFixed(2) ?? '0,00'}', style: pw.TextStyle(fontSize: 8))),
                          pw.Padding(
                              padding: const pw.EdgeInsets.all(4),
                              child: pw.Text('R\$ ${item.valortaxa?.toStringAsFixed(2) ?? '0,00'}', style: pw.TextStyle(fontSize: 8))),
                          pw.Padding(
                              padding: const pw.EdgeInsets.all(4),
                              child: pw.Text('R\$ ${item.valortaxaservico?.toStringAsFixed(2) ?? '0,00'}', style: pw.TextStyle(fontSize: 8))),
                          pw.Padding(
                              padding: const pw.EdgeInsets.all(4),
                              child: pw.Text('R\$ ${item.valoroutros?.toStringAsFixed(2) ?? '0,00'}', style: pw.TextStyle(fontSize: 8))),
                        ],
                      ),
                    ),
                  ],
                  ...
                  children: [
                    pw.Expanded(
                      child: pw.Column(
                        crossAxisAlignment: pw.CrossAxisAlignment.start,
                        children: [
                          pw.Text('Total Tarifa: R\$ ${totalValorHotel.toStringAsFixed(2)}'),
                          pw.Text('Total Taxa: R\$ ${totalValorTaxa.toStringAsFixed(2)}'),
                          pw.Text('Total Serviço: R\$ ${totalValorTaxaServico.toStringAsFixed(2)}'),
                          pw.Text('Total Assento: R\$ ${totalValorOutros.toStringAsFixed(2)}'),
                    ...
                    pw.Expanded(
                      child: pw.Column(
                        children: [
                          ...
                          pw.Text(vendaHotelAtual.solicitante.toString()),
                        ],
                      ),
                    ),
      ...
      await Printing.sharePdf(
        ...
        filename: 'Requisicao_servico.pdf',
      );
    }
  }
  ...
  Widget buildButtonsRow() {
    return Wrap(
      ...
      children: [
        ...
        ElevatedButton(
          onPressed: habilitaSalvarCancelar ? onServico : null,
          style: ElevatedButton.styleFrom(
            backgroundColor: Colors.teal,
            foregroundColor: Colors.white,
          ),
          child: const Text('Add Serviço'),
        ),
        ElevatedButton(
        ...
        ElevatedButton(
          onPressed: habilitaSalvarCancelar ? () => onExcluir(widget.vendahotel!.idvenda!) : null,
          style: ElevatedButton.styleFrom(
              ...
              child: SizedBox(
                ...
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    rows: _itensVendaHotel.map((item) {
                      return DataRow(cells: [
                        DataCell(Row(children: [
                          //IconButton(onPressed: () =>  print('Item clicado: ${item.toJson()}'), icon: const Icon(Icons.edit)),
                          IconButton(onPressed: () =>  _abrirFormularioAddServico(itemvendahotel: item.toJson()), icon: const Icon(Icons.edit)),
