...
import 'package:sistrade/models/vendedor_model.dart';
...
class VendedorForm extends StatefulWidget {
  ...
  @override
  _VendedorFormState createState() => _VendedorFormState();
}
...
class _VendedorFormState extends State<VendedorForm> {
  ...
  @override
  void didUpdateWidget(covariant VendedorForm oldWidget) {
  ...
  Future<void> _carregarDadosIniciais() async {
    ...
    final vend = await EntidadeService.getVendedorById(widget.identidade.toString());

    atualizarvendedorAtual(vend);

    setState(() {
        label1Controller.text = 'Comissão Aereo';
        label2Controller.text = 'Comissão Serviço';
        
        nroController.text = vend.id.toString();
  ...
  void atualizarvendedorAtual(Vendedor novo) {
    setState(() {
      vendedorAtual = novo;
    });
  }
  ...
  void onNovo() {
    ...
    atualizarvendedorAtual(vendedorAux);
  }
        ...
        try{
            ...
            if (_formKey.currentState!.validate()) {
              final vendedor = Vendedor(
                entidadeid: int.tryParse(nroEntidadeController.text) ?? 0,
                id: int.tryParse(nroController.text) ?? 0,
              ...
              bool sucesso = false;
              if ((vendedorAtual.id == null)|| (vendedorAtual.id == 0)) {

                final idGerado = await EntidadeService.createVendedor(vendedor);
                ...
                sucesso = await EntidadeService.updateVendedor(vendedor);
                atualizarvendedorAtual(vendedor); 
  ...
  void onExcluir(int? id) async{
    ...
    if ((vendedorAtual.id != 0)&&(vendedorAtual.id != null)){
      ...
      if (confirmar == true) {
          try {
            if(id != null){
              await EntidadeService.deleteVendedor(id);
  ...
  Widget buildButtonsRow() {
    return Wrap(
      ...
      children: [
        ...
        ElevatedButton(
          onPressed: (habilitaSalvarCancelar) ?  () => onExcluir(vendedorAtual.id!) : null,
          style: ElevatedButton.styleFrom(
