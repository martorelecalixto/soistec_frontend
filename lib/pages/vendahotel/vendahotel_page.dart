...
import 'package:sistrade/models/vendahotel_model.dart';
import 'package:sistrade/services/vendahotel_service.dart';
...
import 'package:sistrade/widgets/vendahotel_form.dart';
...
class VendaHotelPage extends StatefulWidget {
  const VendaHotelPage({
    super.key,
    });

  @override
  _VendaHotelPageState createState() => _VendaHotelPageState();
}

class _VendaHotelPageState extends State<VendaHotelPage> {
  List<Map<String, dynamic>> vendahotel = [];
  List<Map<String, dynamic>> vendahotelFiltradas = [];
  ...
  Future<void> _carregarVendaHotel({bool aplicarFiltros = false}) async {
    ...
    try {
      ...
      final List<VendaHotel> resultado = await VendaHotelService.getVendaHotel(
      ...
      setState(() {
        vendahotel = listaMapeada;
        vendahotelFiltradas = listaMapeada;
        isLoading = false;
      });
    } catch (e) {
  ...
  void _filtrarVendaHotel(String valor) {
    final query = valor.toLowerCase();
    final filtradas = vendahotel.where((item) {
    ...
    setState(() => vendahotelFiltradas = filtradas);
  }


  void _abrirFormularioRequisicaoHotel({Map<String, dynamic>? vendahotel}) async {
  ...
  void _imprimirPDF() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'relatorio_venda_servico.pdf',
    );
  }
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    onChanged: _filtrarVendaHotel,
                  ),
                ),
                const SizedBox(width: 8),
                ElevatedButton.icon(
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.green),
                  onPressed: _carregarVendaHotel,
                ...
                ElevatedButton.icon(
                  ...
                  onPressed: () => _abrirFormularioRequisicaoHotel(),
  ...
  Widget _buildTabelaWeb(TextStyle? textStyle, double largura, double width, double height) {
    if (vendahotelFiltradas.isEmpty) {
              ...
              child: SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  ...
                  child: DataTable(
                    ...
                    rows: vendahotelFiltradas.asMap().entries.map((entry) {
                      final index = entry.key;
                      final vendahotel = entry.value;
                        ...
                        cells: [
                          DataCell(Row(
                            children: [
                              IconButton(
                                ...
                                onPressed: () => _abrirFormularioRequisicaoHotel(vendahotel: vendahotel),
                              ),
                              IconButton(
                                icon: const Icon(Icons.delete, color: Colors.red),
                                onPressed: () => _confirmarExclusao(vendahotel['id']),
                              ),
                            ],
                          )),
                          DataCell(SizedBox(
                            width: 80,
                            child: Text(
                              vendahotel['id'].toString().padLeft(5, '0'),
                          ...
                          if (largura > 800)
                            DataCell(SizedBox(
                              width: 100,
                              child: Text(
                                _formatarData(vendahotel['datavenda'].toString()),
                                style: const TextStyle(fontSize: 12),
                              ),
                            )),
                          if (largura > 900)
                          ...
                          if (largura > 1100)
                            DataCell(SizedBox(
                              ...
                              child: Align(
                                ...
                                child: Text(
                                  _formatarMoeda(vendahotel['valortotal']),
                                  style: const TextStyle(fontSize: 12),
                                ),
                              ),
                            )),
  ...
  Widget _buildListaMobile(TextStyle? textStyle) {
    ...
    return ListView.builder(
      itemCount: vendahotelFiltradas.length,
      itemBuilder: (_, index) {
        final vendahotel = vendahotelFiltradas[index];
        final isEven = index % 2 == 0;
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      icon: const Icon(Icons.delete, color: Colors.red),
                      onPressed: () => _confirmarExclusao(vendahotel['idvenda']),
                    ),
                  ],
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [ // Text('datavenda: ${vendabilhete['datavenda']}'
                      Text(vendahotel['id'].toString().padLeft(5, '0'), style: const TextStyle(fontSize: 12)),
                      Text(vendahotel['entidade'].toString(), style: const TextStyle(fontSize: 12)),
